#!/usr/bin/env bash
# Obsidian-Memory REST API helper functions
...
# Write/create a note
# Usage: mem_write_note <title> <content> <relative_path> [note_type] [project] [tags_csv]
mem_write_note() {
    local title="$1"
    local content="$2"
    local rel_path="$3"
...
# Usage: mem_session_observe <session_id> <event_type> <content>
mem_session_observe() {
    local session_id="$1"
    local event_type="$2"
    local content="$3"
...
# Usage: echo "$response" | check_error
check_error() {
    local response
    response=$(cat)
